...
type Candidatura struct {
	...
	Descricacao       string    `json:"descricao" csv:"desricao"`                    // Descrição da candidatura, informando o tipo e ano.
	Data              time.Time `json:"data" csv:"data"`                             // Data que a eleição ocorreu.
	...
	Situacao          string    `json:"situacao" csv:"situacao"`                     // Situação da candidatura (podendo ser APTO ou INAPTO).
	SituacaoCandidato string    `json:"situacao_candidato" csv:"situacao_candidato"` // Situação do candidato (pondendo ser DEFERIDO ou INDEFERIDO).
	...
	UFOrigem          string    `json:"uf_origem" csv:"uf_origem"`                   // Identificador (2 caracteres) da unidade federatirva de nascimento do candidato.
	MunicipioOrigem   string    `json:"municipio_origem" csv:"municipio_origem"`     // Município de nascimento do candidato.
	...
	Gênero            string    `json:"genero" csv:"genero"`                         // Gênero do candidato (MASCULINO ou FEMININO).
	...
	OcupacaoCandidato string    `json:"ocupacao" csv:"ocupacao"`                     // Ocupação do candidato (como COMERCIANTE).
